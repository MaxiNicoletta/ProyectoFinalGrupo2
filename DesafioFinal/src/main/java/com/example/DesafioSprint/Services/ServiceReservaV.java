...
public class ServiceReservaV implements IServiceReservaV {
    ...
    public ArrayList<ReservaVueloDTO> getAllReservations() {
        ...
        for(ReservaVuelo r: repository.findAll())
            response.add(r.entityToDTO());
        return response;
    }

    @Override
    public FlightResponseDTO updateFlightReservation(Long id,ReservasVueloRequestDTO reservasVueloRequestDTO) {
        ReservaVuelo reservaVuelo = new ReservaVuelo();
        ReservaVuelo updatedReservaVuelo = reservaVuelo.dtoToEntity(reservasVueloRequestDTO.getFlightReservation());
        updatedReservaVuelo.setId(id);
        repository.save(updatedReservaVuelo);
        return new FlightResponseDTO("Vuelo modificado correctamente.");
    }

    @Override
    public FlightResponseDTO deleteFlightReservation(Long id) {
